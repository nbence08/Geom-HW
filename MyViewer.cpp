    ...
    if (model_type != ModelType::BEZIER_SURFACE){
      ...
      for (auto v : mesh.vertices()) {
        ...
        for (auto v : mesh.vertices()) {
            ...
            DoubleMatrix nCoeffs;
            BezierCurve::bernsteinAll(n, uv.first, nCoeffs);
            DoubleMatrix mCoeffs;
            BezierCurve::bernsteinAll(m, uv.second, mCoeffs);
            ...
            for (size_t i = 0; i <= n-1; ++i)
                for (size_t j = 0; j <= m; ++j) {
                    auto idp1 = (i+1) * (m+1) + j;
                    auto id = i * (m+1) + j;
                    vecSu += n*(control_points[idp1] - control_points[id]) * nCoeffs[n-1][i] * mCoeffs[m][j];
                }
            ...
            for (size_t i = 0; i <= n; ++i)
                for (size_t j = 0; j <= m-1; ++j) {
                    auto idp1 = i * (m+1) + (j+1);
                    auto id = i*(m+1) +j;
                    vecSv += m * (control_points[idp1] - control_points[id]) * nCoeffs[n][i] * mCoeffs[m-1][j];
                }
            ...
            for (size_t i = 0; i <= n - 2; ++i)
                for (size_t j = 0; j <= m; ++j) {
                    ...
                    auto idp1 = (i+1) * (m+1) + j;
                    auto id = i*(m+1) +j;
                    vecSuu += n * (n-1)* (control_points[idp2] - 2.0 * control_points[idp1] + control_points[id]) * nCoeffs[n - 2][i] * mCoeffs[m][j];
                }
            ...
            for (size_t i = 0; i <= n; ++i)
                for (size_t j = 0; j <= m-2; ++j) {
                    ...
                    auto idp1 = i * (m+1) + (j+1);
                    auto id = i * (m+1) + j;
                    vecSvv += m*(m-1)* (control_points[idp2] - 2.0 * control_points[idp1] + control_points[id]) * nCoeffs[n][i] * mCoeffs[m-2][j];
                }

            Vec vecSuv;
            for (size_t i = 0; i <= n-1; ++i)
                for (size_t j = 0; j <= m-1; ++j) {
                    auto idp1 = (i+1) * (m+1) + (j + 1);
                    auto id = i * (m+1) + j;
                    vecSuv += n * m * ( control_points[idp1] - control_points[id]) * nCoeffs[n-1][i] * mCoeffs[m-1][j];
                }
            
            Vector su(vecSu.x, vecSu.y, vecSu.z);
            Vector sv(vecSv.x, vecSv.y, vecSv.z);
            Vector suu(vecSuu.x, vecSuu.y, vecSuu.z);
            Vector svv(vecSvv.x, vecSvv.y, vecSvv.z);
            Vector suv(vecSuv.x, vecSuv.y, vecSuv.z);

            auto E = su.dot(su);
            auto F = su.dot(sv);
            auto G = sv.dot(sv);

            auto normal = su.cross(sv).normalize();

            auto L = normal.dot(suu);
            auto M = normal.dot(suv);
            auto N = normal.dot(svv);
...
void MyViewer::elevateDegree() {
    size_t n = degree[0]+1, m = degree[1]+1;
    size_t newn = degree[0]+2, newm = degree[1]+2;
    auto oldCps = control_points;
    decltype(oldCps) newCps;
    
    for (int i = 0; i < n; i++) {

        decltype(oldCps) bigger;

        bigger.reserve(newm+1);
        bigger.push_back(qglviewer::Vec(0.0, 0.0, 0.0));
        for (int k = m*i; k < m*(i+1); k++) bigger.push_back(oldCps[k]);
        bigger.push_back(qglviewer::Vec(0.0, 0.0, 0.0));

        for (int k = 0; k < newm; k++) {
            newCps.push_back(bigger[k] * ((double)k / (double)(m))+
                bigger[k + 1] * (1.0 - (double)k / (double)(m)));
        }
    }
    degree[1]++;
    m = newm;
    ...
    oldCps = newCps;
    newCps.clear();
    newCps.reserve( newn*newm );
    for(int i = 0; i < newn*newm; i++) newCps.emplace_back(qglviewer::Vec());

    for (int i = 0; i < m; i++) {

        decltype(oldCps) bigger;

        bigger.reserve(newn + 1);
        bigger.push_back(qglviewer::Vec(0.0, 0.0, 0.0));
        for (int k = 0+i; k < (n-1)*m + i + 1 ; k+=m) bigger.push_back(oldCps[k]);
        bigger.push_back(qglviewer::Vec(0.0, 0.0, 0.0));

        for (int k = 0; k < newn; k++) {
            newCps[k*m+i] =(bigger[k] * ((double)k / (double)(n)) +
                bigger[k + 1] * (1.0 - (double)k / (double)(n)));
        }
    }
    degree[0]++;
...
void MyViewer::generateMesh(size_t resolution) {
  ...
  for (size_t i = 0; i < resolution; ++i) {
    ...
    for (size_t j = 0; j < resolution; ++j) {
      ...
      uvs[handles[handles.size()-1]] = std::pair<double, double>(u, v);
    }
  }
...
void MyViewer::keyPressEvent(QKeyEvent *e) {
    OpenMesh::Subdivider::Uniform::Sqrt3T<MyMesh> sqrt;
